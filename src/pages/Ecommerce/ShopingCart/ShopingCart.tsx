...
const ShopingCart = () => {
  document.title = "Shop Cart | Ramana - Admin & Dashboard Template";
  ...
  const countUP = (item: any) => {
    setShoppingCount(
      (shoppingCount || [])?.map((count) =>
        count.id === item.id
          ? { ...count, num: item.num + 1, Total: (item.num + 1) * item.price }
          : count
      )
    );
  };
  ...
  const countDown = (item: any) => {
    setShoppingCount(
      (shoppingCount || []).map((count: any) =>
        count.id === item.id && count.num > 0
          ? {
              ...count,
              num: item.num > 0 ? item.num - 1 : 0,
              Total: (item.num > 0 ? item.num - 1 : 0) * item.price,
            }
          : count
      )
    );
                          ...
                          <Table className="table-borderless">
                            <tbody>
                              <tr>
                                ...
                                <td className="fw-semibold text-end cart-subtotal">
                                  ${subTotal || ""}
                                </td>
                              </tr>
                              <tr>
                                <td colSpan={2}>
                                  Discount{" "}
                                  <span className="text-muted">(STEEX30)</span>{" "}
                                  :{" "}
                                </td>
                              ...
                              <tr>
                                ...
                                <td className="text-end cart-tax">
                                  ${tax.toFixed(2) || ""}
                                </td>
                              </tr>
                              <tr>
                                ...
                                <td className="text-end">
                                  <span className="fw-semibold cart-total">
                                    $
                                    {(subTotal + charge + tax - dis).toFixed(
                                      2
                                    ) || ""}
                                  </span>
                                </td>
