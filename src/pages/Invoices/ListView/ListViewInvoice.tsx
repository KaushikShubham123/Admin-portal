...
const ListViewInvoice = () => {
  document.title = "Invoice List | Ramana - Admin & Dashboard Template";
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        accessor: "invoice_no",
        Cell: (cell: any) => {
          return <Link to="#">#TBS{cell.row.original.invoice_no}</Link>;
        },
        Filter: false,
        isSortable: true,
      },
      ...
      },
      {
        Header: "Amount",
        Cell: (cell: any) => {
          return <span>${cell.row.original.invoice_amount}</span>;
        },
        Filter: false,
        isSortable: true,
        disableFilters: true,
      },
      {
        ...
        accessor: (cellProps: any) => {
          switch (cellProps.status) {
            case "Paid":
              return (
                <Badge text="success" bg="success-subtle" className="status">
                  {" "}
                  {cellProps.status}
                </Badge>
              );
            case "Unpaid":
              return (
                <Badge text="danger" bg="danger-subtle" className="status">
                  {" "}
                  {cellProps.status}
                </Badge>
              );
            default:
              return (
                <Badge text="warning" bg="warning-subtle" className="status">
                  {" "}
                  {cellProps.status}
                </Badge>
              );
    ...
    <React.Fragment>
                    ...
                    {(invoice || [])?.map((item: any) => {
                      return (
                        <Col
                          ...
                          sm={6}
                          key={item.id}
                          className="border-end-sm"
                        >
                          <div className="d-flex align-items-center gap-2 mb-4">
                            <div className="avatar-xs flex-shrink-0">
                              <div
                                className={`avatar-title bg-body-secondary text-${item.color} border border-${item.color}-subtle rounded-circle`}
                              >
                          ...
                          <h3 className="mb-0">
                            ...
                            />
                            <small
                              className={`text-${item.iconColor} fs-xs fw-normal ms-1`}
                            >
                              <i
                                className={`${item.statusIcon} align-baseline`}
                              ></i>{" "}
                              {item.amout}%
                            </small>
                          </h3>
              ...
              <Card>
                <Card.Body>
                  ...
                  <OverlayTrigger
                    ...
                    placement="top"
                    overlay={
                      <Tooltip id="tooltip-top"> $234.95 Paid Amount </Tooltip>
                    }
                  >
      ...
      <Modal show={addInvoice} onHide={handleCloseInvoice}>
        <Modal.Header closeButton>
          <Modal.Title>
            {editInvoice ? "update Invoice List" : "Add Invoice List"}
          </Modal.Title>
        </Modal.Header>
        <Form autoComplete="off" onSubmit={formik.handleSubmit}>
          <Modal.Body>
            <div className="mb-3">
              <Form.Label htmlFor="Customer-Name-input">
            ...
            <Row>
              ...
              <Col lg={6}>
                <div className="mb-3">
                  <Form.Label htmlFor="Amount-input">
