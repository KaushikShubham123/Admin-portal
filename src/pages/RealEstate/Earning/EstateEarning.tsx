...
import { getEarningList as onGetEarningList } from "slices/thunk";
import EarningChart from "./EarningChart";
import TableContainer from "Common/TableContainer";
import { useDispatch, useSelector } from "react-redux";
import { createSelector } from "reselect";
const Earning = () => {
  document.title = "Earnings | Ramana - Admin & Dashboard Template";

  const dispatch = useDispatch<any>();

  const selectEarningList = createSelector(
    (state: any) => state.RealEstate.earninglist,
    (earningList) => earningList
  );

  const { earningList } = useSelector((state: any) => ({
    earningList: selectEarningList(state),
  }));
  ...
  const column = useMemo(
    () => [
      {
        Header: <Form.Check type="checkbox" id="checkbox-all" />,
        accessor: (cell: any) => {
          return (
            <Form.Check>
      ...
      },
      {
        ...
        Cell: (cell: any) => {
          return (
            <span className={`in_out text-${cell.row.original.color} fs-md`}>
      ...
      },
      {
        ...
        Cell: (cell: any) => {
          return (
            <Link
              to="/apps-real-estate-agencies-overview"
      ...
      },
      {
        ...
        isSortable: true,
        Cell: (cell: any) => {
          return <span>{cell.row.original.details} </span>;
        },
      },
      {
        Header: "Type",
          ...
          <Row className="row-cols-xxl-5 row-cols-lg-3 row-cols-md-2 row-cols-1">
            {(earningcard || [])?.map((item: any) => {
              return (
                <Col key={item.id}>
                  <Card
                    className={`border-bottom border-2 card-animate border-${item.border}`}
                  >
                    <Card.Body>
                      <Badge
